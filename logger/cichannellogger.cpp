...
cilog_date_hour_based_backup_backend::cilog_date_hour_based_backup_backend(
...
void cilog_date_hour_based_backup_backend::consume(
...
void cilog_date_hour_based_backup_backend::rotate_file() {
  if (file_.is_open()) {
    file_.close();
    file_.clear();
  }
...
cilog_date_hour_based_backup_backend::generate_active_filepath() {
  // e.g. example.log
...
void cilog_date_hour_based_backup_backend::rollover_without_max() {
  try {
...
boost::filesystem::path
cilog_date_hour_based_backup_backend::generate_filepath() {
...
std::string cilog_date_hour_based_backup_backend::datetime_string_with_format(
    std::string const& format, int hour) {
...
std::string cilog_date_hour_based_backup_backend::get_current_date_hour() {
  return datetime_string_with_format("%Y-%m-%d %H", 0);
}

////////////////////////////////////////////////////////////////////////////////

cilog_size_based_backup_backend::cilog_size_based_backup_backend(
    boost::filesystem::path const& target_path,
...
void cilog_size_based_backup_backend::consume(
    boost::log::record_view const& /*rec*/,
    std::basic_string<char> const& formatted_message) {
...
void cilog_size_based_backup_backend::rotate_file() {
  if (file_.is_open()) {
    file_.close();
    file_.clear();
  }
...
boost::filesystem::path
cilog_size_based_backup_backend::generate_active_filepath() {
  // e.g. example.log
  std::stringstream filename_ss;
  filename_ss << file_name_suffix_;
  filename_ss << ".log";
...
void cilog_size_based_backup_backend::rollover_without_max() {
  try {
    boost::filesystem::path backup_file_path = generate_filepath();
    boost::filesystem::create_directories(backup_file_path.parent_path());
    boost::filesystem::rename(file_path_, backup_file_path);
...
boost::filesystem::path cilog_size_based_backup_backend::generate_filepath() {
...
std::string cilog_size_based_backup_backend::datetime_string_with_format(
...
uintmax_t cilog_size_based_backup_backend::scan_next_index(
    boost::filesystem::path const& path, boost::regex const& pattern) {
...
uintmax_t cilog_size_based_backup_backend::parse_index(
